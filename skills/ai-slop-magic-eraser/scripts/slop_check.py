...
Covers only what is mechanically decidable: symbols, fixed phrases, and a few
shapes with low false-positive rates. Cadence, structure and factual errors need
judgment and live in the skill's procedure and references/tells.md.
...
RULES: list[tuple[str, re.Pattern, str, bool]] = [
    (
        "symbol",
        re.compile("—"),
        "em dash; use a spaced hyphen ' - ' or restructure",
        True,
    ),
    (
        "symbol",
        re.compile("–"),
        "en dash; use a hyphen, or 'to' in a range",
        True,
    ),
    ("symbol", EMOJI, "emoji; cut it", True),
    (
        "symbol",
        re.compile("[“”‘’]"),
...
def apply_fix(path: Path) -> int:
    """Replace only the unambiguous characters. Never touches fenced code."""
    raw = path.read_text(encoding="utf-8")
    ...
    for line, code in zip(lines, in_code):
        if code:
            ...
            continue
        new = line
        for bad, good in MECHANICAL_FIX.items():
            if bad in new:
                changed += new.count(bad)
                new = new.replace(bad, good)
        stripped = EMOJI.sub("", new)
        if stripped != new:
            changed += len(new) - len(stripped)
            new = re.sub(r"[ \t]{2,}", " ", stripped)
        out.append(new)
...
def main() -> int:
    ...
    ap.add_argument(
        "--fix",
        action="store_true",
        help="rewrite dashes, curly quotes and emojis; leaves judgment calls alone",
    )
