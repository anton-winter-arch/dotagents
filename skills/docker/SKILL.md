---
name: docker
description: Scaffolds, operates, reviews, and hardens containerized projects - from a cold start or inside an existing repo. Use when containerizing an app, when authoring or editing any Dockerfile/Containerfile/compose file, when inheriting an unfamiliar repo that has one and needing to orient in it, when a build is slow, an image is fat, or the layer cache never hits, when a container runs as root or leaks secrets into layers, when a container will not start or dies mysteriously (137/OOM, 126, 127, exec format error), when debugging a running container, reclaiming Docker disk, or pushing to a registry, and when reviewing container config before it ships. Also use before granting a container ANY host privilege - Docker socket, privileged, host network, bind-mounting / - those are host-escape grants and warrant a hard stop. Ships a deterministic linter (scripts/docker_check.py) for the checks hadolint and trivy do not make, and drives those mature tools rather than duplicating them.
license: MIT
---
