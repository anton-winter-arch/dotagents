...
description: Data platform engineering on Databricks, Microsoft Fabric, Snowflake and open source. Use when choosing a platform, layering or boundaries; when onboarding a source and picking how to land it, or weighing log-based CDC against cursor polling; when a dbt or SQLMesh project is laid out or reviewed; when an incremental model leaves stale rows behind or a MERGE runs far too long; when an unfamiliar source needs profiling before committing to it; when deciding which quality checks to run and where they go; when a bill grew or spend must be forecast or capped; when catalogs, grants, tags, masking or row filters are set up; when records from several systems must resolve to one; when jobs are promoted between environments with Terraform or asset bundles; and when inheriting someone else's platform and judging it. MUST be used when rows go missing or duplicate while every run reports green. Routes grain, SCD and conformance to dimensional-data-modeling, RAG and model serving to ai-engineering, image builds to docker.
---
