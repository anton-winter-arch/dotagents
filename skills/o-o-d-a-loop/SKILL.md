...
description: >-
  ...
  loop plus the prescriptive decision corpus: decision theory, game theory,
  mechanism design, systems thinking, and behavioral bets. Classifies the decision
  (reversibility, stakes, emotional load, time pressure), routes to the right
  lens, and exits with a committed act, an installed policy, or a scheduled
  decision - never "think about it more". Use when the user owns a live choice
  under uncertainty with real stakes - strategic ("should I take this job", "help
  me decide"), technical ("which database/architecture", "build vs buy", "migrate
  or rewrite"), or both; when asked to stress-test, tear apart, or
  devil's-advocate a plan, idea, design, or RFC; when the user keeps circling
  without converging (rumination, analysis paralysis); when asked how to get
  better at a recurring activity (capability audit); or to review a past
  decision without outcome bias. Not for factual questions, already-made
  decisions, or trivial reversible picks.
---
