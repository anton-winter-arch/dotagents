...
| `description` | 1-1024 chars, **third person**, states what the skill does AND the concrete conditions that should trigger it, no XML tags |
...
2. `description` non-empty, <=1024 chars, third person, has explicit
   when-to-fire conditions, no XML.
...
4. **Evals written AND run** - `evals/evals.json` exists per the convention
   above (min 3 cases, both polarities) and the trigger evals have actually
   been run with the model pinned; failures are fixed or explicitly waived by
   the user, not ignored.
